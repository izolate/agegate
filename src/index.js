...
class AgeGate {
  constructor(opts, cb) {
    ...
    if (this.isEnabled.data) {
      this.validateData(opts.data);
    }

    // render
    if (this.isEnabled.countries) {
      this.populate();
    }
  ...
  validateData(data) {
    ...
    ok =
      ok &&
      ["code", "name", "age"].every((k) => data[random].hasOwnProperty(k));
  ...
  saveCookie(expiry = null) {
    ...
    cookies.setItem(
      this.options.cookieName || "old_enough",
      true,
      expiry,
      path,
      domain
  ...
  respond(success = false, message = "Age verification failure") {
    this.callback(success ? null : new Error(`[AgeGate] ${message}`));
  }
}
